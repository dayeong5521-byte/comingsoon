...
catch(e) { console.warn('Analytics 초기화 실패:',e.message); analytics={logEvent:function(){}}; }
...
var keywordButtonMode='search'; // search | loading | save | saved
...
function formatDateDot(dateStr){
  ...
  if(dateStr.includes('~')){
    ...
    if(/^\d{1,2}$/.test(end)) return startFmt+'~'+end.padStart(2,'0');
    var startP=start.split('-'), endP=end.split('-');
    if(startP[0]===endP[0]&&startP[1]===endP[1]) return startFmt+'~'+endP[2];
...
/* ── 키워드 버튼 상태 관리 ──
   ...
   저장 완료/이미 저장: 이미 저장됨
── */
function setKeywordButton(mode){
  keywordButtonMode=mode;
  var btn=document.getElementById('kwSaveBtn');
  if(!btn) return;

  if(mode==='search'){
    btn.disabled=false;
    btn.textContent='키워드 검색';
...
function handleKeywordButton(){
  if(keywordButtonMode==='search'){
    startHunt();
    return;
  }
  if(keywordButtonMode==='save'){
    saveCurrentKeyword();
    return;
  }
}
...
function bindCardEvents(container){
  ...
  container.querySelectorAll('.pcard-img-wrap:not([data-link])').forEach(function(el){
    ...
    el.addEventListener('click',function(){
      ...
      var brand=card&&card.querySelector('.pc-brand')&&card.querySelector('.pc-brand').textContent||'';
      var name=card&&card.querySelector('.pc-name')&&card.querySelector('.pc-name').textContent||'';
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if(ms){
    ...
    ms.oninput=function(){
      currentSearchItems=[];
      setKeywordButton('search');
      if(currentView==='radar') renderItems([]);
    };
  }

  var btnGridArc=document.getElementById('btnGridArc');
  var btnListArc=document.getElementById('btnListArc');
  if(btnGridArc) btnGridArc.onclick=function(){ viewMode='grid'; btnGridArc.classList.add('on'); if(btnListArc) btnListArc.classList.remove('on'); renderArchive(); };
  if(btnListArc) btnListArc.onclick=function(){ viewMode='list'; btnListArc.classList.add('on'); if(btnGridArc) btnGridArc.classList.remove('on'); renderArchive(); };
